...
def filter_by_criterion(rows: List[str],
                        criterion: Callable[[int, int], int],
                        bit_position: int = 0) -> str:
    if len(rows) <= 1:
        return rows[0]
...
def solve_2(data: str) -> int:
    ...
    def o2_criterion(zeroes, ones): return '0' if zeroes > ones else '1'
    def co2_criterion(zeroes, ones): return '0' if zeroes <= ones else '1'
